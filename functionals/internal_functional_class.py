...
class InternalPotential:
    ...
    def __init__(self, functional: Union[str,Array] = 'entropy', coeff: float = 1.0, sigma: Optional[float] = 1.0, method: str = 'exact', prob_dim=  None):
        """
        ...
        for func in functional:
            if func not in ['entropy', 'fisher']:
                raise ValueError("Unsupported functional type. Choose either 'entropy' or 'fisher'.")
            if func == 'entropy' and method not in ['exact', 'hutchinson']:
                raise ValueError("Unsupported method for entropy functional. Choose either 'exact' or 'hutchinson'.")
            if func == 'fisher' and method not in ['exact','autodiff'] or coeff <= 0:
                raise ValueError("Unsupported method for fisher functional. Choose either 'exact' or 'autodiff'. Coefficient must be positive.")
    ...
    def __call__(self,parametric_model: ParametricModel, z_samples: Array,z_trajectory: Optional[Array] = None,time_steps: Optional[Array] = None, 
        ...
        if 'entropy' in self.functional:
            ...
            entropy = parametric_model.log_likelihood(t = time_steps,
                                          ...
                                          method = self.method,
                                          params = params,
                                          log_trajectory = False) # (batch_size,)
            ...
            score = parametric_model.score_function(t = time_steps,
                                        ...
                                        method = self.method,
                                        params = params,
                                        score_trajectory = False) # (batch_size, dim)
            # Compute fisher information
            fisher_info = self.sigma**4/8 *jnp.mean(jnp.linalg.norm(score,axis=-1)**2,axis=0)  # Return the mean fisher information
