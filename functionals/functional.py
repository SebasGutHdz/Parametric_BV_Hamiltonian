...
class Potential:
    ...
    def evaluate_energy(self,node:NeuralODE,z_samples:Array,params:Optional[PyTree]=None)-> float:
        '''
        ...
        z_trajectory,time_steps = node(z_samples, history=True, params = params) # (batch_size, time_steps, dim)
        ...
        if self.linear is not None:
            linear_energy,_ = self.linear.evaluate_energy(node,z_samples,x_samples=x_samples)
            linear_energy = linear_energy*self.linear.coeff
            energy += linear_energy
        # Internal potential
        if self.internal is not None:
            internal_energy = self.internal(node=node,
                                            z_samples=z_samples,
    ...
    def compute_energy_gradient(self,node:NeuralODE,z_samples: Array,params: PyTree) -> PyTree:
        '''
        ...
        def energy_evaluation(p:PyTree)-> Array:
            # Transform reference samples
            z_trajectory,time_steps = node(z_samples, history=True, params = p) # (batch_size, time_steps, dim)
            x_samples = z_trajectory[:,-1,:]    
            energy = 0.0
            linear_energy = 0.0
            internal_energy = 0.0
