...
def visualize_gradient_flow_results(results: dict, figsize: tuple = (15, 10)):
    ...
    x_min,x_max = min(initial_samples[:,0].min(),final_samples[:,0].min()),max(initial_samples[:,0].max(),final_samples[:,0].max())
    y_min,y_max = min(initial_samples[:,1].min(),final_samples[:,1].min()),max(initial_samples[:,1].max(),final_samples[:,1].max())
