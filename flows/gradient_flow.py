...
def gradient_flow_step(node: nnx.Module, z_samples: Array, G_mat: G_matrix,
                                    potential: Potential, device, step_size: float = 0.01,
    ...
    eta, solver_info = G_mat.solve_system(z_samples_g_mat, neg_energy_grad,
...
def run_gradient_flow(node: nnx.Module, z_samples: Array, G_mat: G_matrix,
    ...
    with jax.default_device(device):
        ...
        for iteration in p_bar:
            ...
            if iteration == 0:
                _, samples0, _, _, _ = potential.evaluate_energy(current_node, z_samples)
            ...
            if (iteration % progress_every == 0 and iteration > 0) or iteration == max_iterations - 2:
                current_energy, samples1,_,_,_ = potential.evaluate_energy(current_node, z_samples, current_params)
